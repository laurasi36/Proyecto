    ...
    <main>
        <section class="container " id="colores">
            <?php
                ...
                $sql = "SELECT * FROM infojunior WHERE id = :id_playero";
                $sql2 = "SELECT * FROM coloresjunior WHERE codigo = :id_playero";
                ...
                //Miramos si hemos seleccionado los playeros con esos id
                if (in_array($id_playero, array(7, 8, 15, 16, 23, 24, 31, 32, 39, 40))){
                    //Si es asi mostramos unos botones para elegir la talla
                    for($i=17; $i<=26; $i++){
                        echo "<button class='boton' value='$i' onclick='selectSize($i)'>EUR $i</button>";
                    }
                }else{
                    //Si se escogen otros id, se muestran otros botones para la talla
                    for($i=27; $i<=36; $i++){
                        echo "<button class='boton' value='$i' onclick='selectSize($i)'>EUR $i</button>";
                    }
                }
